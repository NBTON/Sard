...
_reg(CapabilitySpec(
    ...
    timeout_seconds=10,
    retry_policy="no retry; empty query falls back to first 4 heritage events",
    fallback="orchestrator uses HERITAGE_EVENTS_DATABASE[:4] when search yields empty; Google Calendar URL still produced per event",
    progress_stages=_stages("intent", "plan", "typed_content", "rendering", "validation", "storage", "verified_result"),
    support_status=SupportStatus.SUPPORTED,
...
_reg(CapabilitySpec(
    ...
    timeout_seconds=60,
    retry_policy="provider: no retry beyond single call; immediate core_fallback on missing key or error",
    fallback="core_fallback: Hasawi oasis oral history template with 2 mock diarized segments (offline deterministic)",
    progress_stages=_stages("intent", "tools/inputs", "typed_content", "grounding", "validation", "verified_result"),
    support_status=SupportStatus.LIMITED,
    limitations="Offline transcription is deterministic template (not speech-to-text). Provider path is request/response, not streaming. No speaker diarization guarantees offline.",
...
_reg(CapabilitySpec(
    ...
    timeout_seconds=45,
    retry_policy="provider: single POST to DashScope; fallback to core_fallback on non-200 or exception",
    fallback="core_fallback: templated cultural artifact description + local EXIF/dimensions probe",
    progress_stages=_stages("intent", "tools/inputs", "typed_content", "grounding", "validation", "verified_result"),
    support_status=SupportStatus.LIMITED,
