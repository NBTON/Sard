...
_FRESH_KEYWORDS = re.compile(r"(اليوم|غدا|غدًا|الآن|موعد|فعالية|مهرجان|افتتاح|إغلاق|سعر|تذكرة|جدول|مواعيد|opening|hours|price|event|today|tomorrow)", re.I)
_ITINERARY_KEYWORDS = re.compile(r"(برنامج|مسار|خطة|جدول|يومين|ثلاثة أيام|itinerary|trip|route|plan)", re.I)
_TRANSLATION_HINT = re.compile(r"(ترجم|translate)", re.I)
...
_VISION_HINT = re.compile(r"(صورة|image|photo|vision)", re.I)

def classify_capability(question: str) -> Capability:
    q = question.strip().lower()
    if _VISION_HINT.search(q):
...
_CAPABILITY_REQUIREMENTS: Dict[Capability, Dict[str, bool]] = {
    ...
    Capability.VISION: {"supports_tools": False},  # vision modality check outside
